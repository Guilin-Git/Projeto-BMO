...
scrape_objects.py
------------------
Scraper de alta qualidade para objetos/itens de Hora de Aventura (PT-BR).
Foco em dados limpos e bem estruturados para RAG.

Descobre objetos automaticamente via Categoria:Objetos (com paginação).
Captura TODAS as seções h2 de cada página sem lista fixa de seções.

Saída: knowledge/bronze/items/<Nome>.md
"""
...
OUTPUT_DIR = os.path.join(
    os.path.dirname(__file__), "..", "knowledge", "bronze", "items"
)

# Seções de navegação/referência a ignorar
SECOES_IGNORAR = {
...
# ---------------------------------------------------------------------------
# Extração da infobox de objetos
# ---------------------------------------------------------------------------
...
def extrair_infobox_objeto(soup) -> dict:
    """Extrai campos da infobox de objetos do Fandom."""
    dados = {}
    ...
    for item in infobox.find_all("div", class_="pi-item"):
        ...
        # Tenta match exato, depois match parcial
        chave = MAPA_LABELS_OBJETO.get(label)
        if not chave:
            for k, v in MAPA_LABELS_OBJETO.items():
                if k in label:
                    chave = v
                    break
...
# ---------------------------------------------------------------------------
# Descoberta de objetos via página de categoria (com paginação)
# ---------------------------------------------------------------------------
...
def scrape_links_categoria(page: Page) -> list[dict]:
    """
    Navega pela Categoria:Objetos com paginação e coleta todos os links.
    ...
    print(f"  → {len(todos)} objetos encontrados\n")
    return todos

# ---------------------------------------------------------------------------
# Scraping de um objeto
# ---------------------------------------------------------------------------

def scrape_objeto(page: Page, meta: dict) -> dict:
    """Visita a página do objeto e extrai infobox + todas as seções."""
    url = meta["url"]
    page.goto(url, wait_until="domcontentloaded", timeout=30000)
    try:
    ...
    resultado["infobox"] = extrair_infobox_objeto(soup)
...
def gerar_markdown(obj: dict) -> str:
    nome    = obj["nome"]
    ...
    for campo in ["tipo_objeto", "dono", "criador", "material", "poderes",
                  "status", "localizacao", "primeira_aparicao", "ultima_aparicao"]:
        val = infobox.get(campo, "")
        if val:
            linhas.append(f'{campo}: "{val}"')

    linhas.append(f'url: "{obj["url"]}"')
    linhas.append("---")
    linhas.append("")

    # Frase âncora para o embedding
    linhas.append(f"> {nome} é um objeto/item de Hora de Aventura.")
    linhas.append("")
    ...
    # Todas as seções descobertas automaticamente
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Scraping objetos para RAG")
    ...
    with sync_playwright() as pw:
        ...
        # 1. Descobre todos os objetos via categoria
        objetos_meta = scrape_links_categoria(page)
        total = len(objetos_meta)

        # 2. Scrape individual de cada objeto
        for i, meta in enumerate(objetos_meta, 1):
            print(f"[{i:03d}/{total}] {meta['nome']}")
            try:
                dados = scrape_objeto(page, meta)
                todos.append(dados)
    ...
    # 3. JSON consolidado
    json_path = os.path.join(OUTPUT_DIR, "objetos_rag.json")
